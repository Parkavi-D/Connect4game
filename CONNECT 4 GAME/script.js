...
document.querySelectorAll(".column").forEach((e) => {
    e.addEventListener("click", () => {
        ...
        if (sum <= 6 && turn % 2 != 0) {
            document.getElementById(`${e.id}r${sum}`).style.backgroundColor = "white"
            turn++
            check('white')
            document.getElementById("whosturn").innerText = "greenyellow's Turn"
        }
        
        else if (sum <= 6 && turn % 2 == 0) {
            document.getElementById(`${e.id}r${sum}`).style.backgroundColor = "greenyellow"
            turn++
            check('greenyellow')
            document.getElementById("whosturn").innerText = "White's Turn"
